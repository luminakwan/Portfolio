...
const carouselRows = ['第一行', '第二行', '第三行', '第四行', '第五行', '第六行'].map((folder) => ({
  ...
  images: Object.entries(carouselAssets)
    ...
    .map(([path, src]) => ({ src, name: path.split('/').at(-1).replace(/\.[^.]+$/, '') })),
}));
const services = [
