...
for (const folder of await readdir(path.join(root, '首页轮播图片'))) {
  ...
  for (const name of await readdir(directory)) {
    ...
    const thumbnail = await sharp(source).rotate().resize({ width: 900, height: 900, fit: 'inside', withoutEnlargement: true }).webp({ quality: 78 }).toBuffer({ resolveWithObject: true });
    ...
    carousel[`../首页轮播图片/${folder}/${name}`] = { src: `thumbnails/${filename}`, width: thumbnail.info.width, height: thumbnail.info.height };
